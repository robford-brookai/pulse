"""Throwaway-cluster Postgres fixtures for the billing-engine migration tests.

Mirrors `packages/pulse-ledger/tests/conftest.py`: a session-lived Postgres cluster on a
private unix socket only (no TCP listener, so the no-live-network posture holds), a fresh
database per test. Set PULSE_PG_BINDIR to point at a directory with initdb/pg_ctl/postgres if
discovery via PATH or the usual Homebrew/Debian install locations does not find one; with no
server available the Postgres-backed tests skip, visibly, rather than fake the store.
"""
...
def pg_server(tmp_path_factory: pytest.TempPathFactory) -> Iterator[dict[str, str]]:
    ...
    socket_dir = tempfile.mkdtemp(prefix="billing-pg-")
...
def pg_database(pg_server: dict[str, str]) -> Iterator[dict[str, str]]:
    ...
    name = f"billing_engine_test_{next(_db_counter)}"
