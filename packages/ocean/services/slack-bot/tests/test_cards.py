...
from src.cards import alert_card, claimed_card, resolved_card
...
class TestAlertCard:
    def test_returns_exactly_6_blocks(self):
    ...
    def test_ai_summary_block_contains_label(self):
        blocks = make_alert_card()
        ai_block = blocks[3]
        assert ai_block["type"] == "section"
        assert "🤖 AI Summary" in ai_block["text"]["text"]

    def test_ai_summary_block_wraps_summary_in_underscores(self):
        blocks = make_alert_card()
        ai_block = blocks[3]
        assert f"_{AI_SUMMARY}_" in ai_block["text"]["text"]

    def test_actions_block_has_correct_block_id(self):
        blocks = make_alert_card()
        actions = blocks[5]
    ...
    def test_dividers_at_positions_2_and_4(self):
        blocks = make_alert_card()
