...
_HEARTBEAT = _ROOT / "services" / "hubspot-connector" / "src" / "heartbeat.py"
_MAIN = _ROOT / "services" / "hubspot-connector" / "src" / "main.py"


def test_heartbeat_module_exists():
    assert _HEARTBEAT.exists(), "heartbeat.py must exist in hubspot-connector"
...
def test_main_starts_heartbeat_task():
    ...
    assert "hubspot-connector" in src, "Heartbeat must identify as hubspot-connector"


def test_health_endpoint_exists():
    src = _MAIN.read_text()
    assert '"/health"' in src, "Must expose /health endpoint"
    assert '"hubspot-connector"' in src, "Health must return service name"
