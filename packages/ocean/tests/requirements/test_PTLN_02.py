...
class TestPatientTimelineHasura:
    ...
    def test_pg_track_table_call_exists(self):
        ...
        source = _APPLY_METADATA.read_text()
        assert "pg_track_table" in source, (
            "No pg_track_table call found in apply_metadata.py"
        )

    def test_migration_file_exists(self):
        """Migration 0013 must exist for the view patient_timeline."""
        assert _MIGRATION.exists(), (
            f"Migration file not found: {_MIGRATION}"
        )
    ...
    def test_view_columns_in_migration(self):
        ...
        for col in _VIEW_COLUMNS:
            if col in ("event_type", "event_id", "summary"):
                assert f"AS {col}" in source, (
                    f"Missing AS {col} alias in migration view definition"
                )
            else:
                assert col in source, (
                    f"Missing column {col} in migration view definition"
                )
    ...
    def test_no_relationships_for_view(self):
        ...
        if arr_idx != -1 and obj_idx != -1:
            ...
            assert "patient_timeline" not in rel_source, (
                "patient_timeline should not have relationships defined"
            )
