...
def test_outcome_recorded_in_consumer_source():
    ...
    assert '"outcome.recorded"' in src, "consumer.py must register outcome.recorded in EVENT_HANDLERS"
...
def test_event_handlers_contains_outcome_recorded():
    ...
    for node in ast.walk(tree):
        ...
        if target_name and isinstance(value, ast.Dict):
            ...
            assert "outcome.recorded" in keys, "EVENT_HANDLERS dict must contain 'outcome.recorded' key"
            return
