...
actor (ADR-0003: attribution is authentication). Every command's payload SHALL carry
message-level provenance: a reference to the source landing row (its message or event
identifier) sufficient to trace a recorded consent state back to the Customer.io message that
produced it.
