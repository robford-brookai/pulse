...
`deps` names task numbers in this file; `—` means no dependency. Default model is `sonnet`
...
- [ ] 1.1 Rotate every credential in the source repo's tracked `.env`, and record the rotation as
...
- [ ] 1.2 Import `robford-brookai/ocean` at `7bc9d2c` to `packages/ocean` via `git-filter-repo`
...
- [ ] 1.3 Conform `packages/ocean` to the monorepo toolchain (ruff, mypy, pytest, workspace
...
- [ ] 2.1 Establish the topic → `(source, detail-type)` mapping as one generated surface: a source
...
- [ ] 2.2 Replace `libs/ocean-broker`'s Kafka config builders with `EventBridgePublisher`:
...
- [ ] 3.1 `graph-projection/src/handlers/outcomes.py:44` and `:103` — replace the unguarded
...
- [ ] 3.2 `graph-projection/src/handlers/interactions.py:36` and `:72` — replace the
      `last_event_id IS DISTINCT FROM` predicate with a sequence guard. Dedup is not ordering.
      `[model: opus | deps: 1.3 | lane: repo_change | wave: 2a]`
- [ ] 3.3 `graph-projection/src/handlers/logistics.py:125` (`device_associations`) — same
      dedup-only predicate, same replacement.
      `[model: opus | deps: 1.3 | lane: repo_change | wave: 2a]`
- [ ] 3.4 `graph-projection/src/handlers/signals.py:59` — add a guard to the unguarded
...
- [ ] 3.5 Add a sequence column to `slack-bot`'s stored message record and guard `chat_update` on
...
- [ ] 3.6 Re-confirm `control-plane`'s ordering verdict per handler in `EVENT_HANDLERS` and record
...
- [ ] 4.1 `services/github-connector/src/producer.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.2 `services/hubspot-connector/src/producer.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.3 `services/impilo-connector/src/producer.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.4 `services/pocar-connector/src/producer.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.5 `services/zcc-connector/src/producer.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.6 `services/mongodb-connector/src/publisher.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
...
- [ ] 4.7 `services/control-plane/src/producer.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.8 `services/linear-connector/src/producer.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.9 `services/agent-worker/src/publisher.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.10 `services/call-simulator/src/publisher.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.11 `services/sim-driver/src/publisher.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.12 `services/slack-bot/src/publisher.py` `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2b]`
- [ ] 4.13 `services/warehouse-sync/src/main.py` — inline `Producer` used for dead-letter writes.
...
- [ ] 5.1 `services/event-store/src/consumer.py` — verdict order-tolerant (append-only,
      `ON CONFLICT (event_id) DO NOTHING`).
      `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2c]`
- [ ] 5.2 `services/agent-worker/src/consumer.py` — verdict order-tolerant (single event type, one
      source). Its cross-replica `claimed_tasks` duplicate hazard predates this change; note it in
      the HANDOFF, do not fix it here.
      `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2c]`
- [ ] 5.3 `services/call-simulator/src/consumer.py` — verdict order-tolerant (single topic, single
      dispatch per approval).
      `[model: sonnet | deps: 2.2 | lane: repo_change | wave: 2c]`
- [ ] 5.4 `services/control-plane/src/consumer.py` — verdict per 3.6.
      `[model: sonnet | deps: 2.2, 3.6 | lane: repo_change | wave: 2c]`
- [ ] 5.5 `services/graph-projection/src/consumer.py` — convert only; all guard work landed in
      3.1–3.4.
      `[model: sonnet | deps: 2.2, 3.1, 3.2, 3.3, 3.4 | lane: repo_change | wave: 2c]`
- [ ] 5.6 `services/slack-bot/src/consumer.py` — convert only; guard landed in 3.5.
      `[model: sonnet | deps: 2.2, 3.5 | lane: repo_change | wave: 2c]`
- [ ] 5.7 `services/warehouse-sync/src/main.py` — inline `AIOConsumer` to SQS receive/delete.
      `[model: sonnet | deps: 2.2, 4.13 | lane: repo_change | wave: 2c]`

## 6. Wave 3 — infrastructure

- [ ] 6.1 Delete `infra/terraform/modules/msk-ocean/` and add the EventBridge bus.
      `[model: sonnet | deps: 2.1 | lane: repo_change | wave: 3]`
- [ ] 6.2 Add one rule and one SQS queue per consumer, patterns generated from 2.1. Test: each
      rule's pattern matches exactly its consumer's domain set.
      `[model: sonnet | deps: 6.1 | lane: repo_change | wave: 3]`
- [ ] 6.3 Add a DLQ and redrive policy per queue, with dead-letter volume exposed to monitoring
...
- [ ] 6.4 Add the bus archive with retention. This is where ADR §4.6's replay stops being an
      assumption. Retention value per design Open Questions — any value 30–90 days satisfies the
      spec.
      `[model: sonnet | deps: 6.1 | lane: repo_change | wave: 3]`
- [ ] 6.5 Replace `redpanda`, `redpanda-console` and `redpanda-init` in `infra/docker-compose.yml`
...
- [ ] 6.6 Remove `confluent_kafka` from every package manifest and lockfile; add the AWS client
...
- [ ] 7.1 Delete `infra/redpanda/connect.yaml` and the `ocean.warehouse-dlq` topic; move warehouse
      delivery onto the `warehouse-sync` queue from 6.2.
      `[model: sonnet | deps: 5.7, 6.2 | lane: repo_change | wave: 4]`
- [ ] 7.2 Point warehouse dead-lettering at the `warehouse-sync` queue's DLQ. Test: a repeatedly
      failing event lands there and is observable like any other consumer's.
      `[model: sonnet | deps: 7.1, 6.3 | lane: repo_change | wave: 4]`
- [ ] 7.3 Assert warehouse append semantics: out-of-order delivery yields identical table contents,
      and redelivery creates no duplicate row.
      `[model: sonnet | deps: 7.1 | lane: repo_change | wave: 4]`

## 8. Equivalence gate

- [ ] 8.1 Build the equivalence harness: capture graph tables and `audit_log` after a
      ...
      gate proves nothing.
- [ ] 8.2 Run the harness against the Kafka path and the LocalStack path and record the comparison.
