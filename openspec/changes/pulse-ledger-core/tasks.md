...
- [ ] 1.1 Scaffold `packages/pulse-ledger` and `packages/pulse-core` as workspace members
...
- [ ] 1.2 The `ledger` schema, alembic sequence `0001`: `events` (bitemporal, evidence_class,
...
- [ ] 2.1 Catalog → command-type generator: transition tables + Pydantic command types
...
- [ ] 3.1 Command validation core: legality against the generated adjacency for all six subject
...
- [ ] 3.2 Commit path: event + current_state + outbox in one transaction; server-set
...
- [ ] 3.3 Idempotency: D16 key derivation helper in `pulse_core`, unique constraint handling,
...
- [ ] 3.4 Auth and attribution: per-writer bearer credentials → `writer_id` = actor; body-actor
...
- [ ] 3.5 Backfill mode: `POST /commands:batch`, `backfill_genesis` and `reconstruction_gap`
...
- [ ] 4.1 Read APIs: current-state enumeration (subject_type × state), identity candidate lookup
...
- [ ] 4.2 Writer-state cursors: `PUT/GET /writers/{writer_id}/cursor` + `pulse_core` accessors.
      Test: crash/resume round-trip.
      `[model: sonnet | deps: 3.4 | lane: repo_change | wave: 3]`
- [ ] 4.3 `pulse_core` client: command submission with response classification
...
- [ ] 4.4 Outbox relay worker through `ocean-broker`'s `EventBridgePublisher`: per-subject
...
- [ ] 4.5 LocalStack wiring: ledger Postgres + relay + bus in `infra/docker-compose` extension of
...
- [ ] 5.1 End-to-end fold equivalence: commit a mixed history (forward, backdated, reversal,
...
- [ ] 5.2 Pin the downstream names: update `design/delivery/pulse-s1-work-orders.md` "confirm
