# DX rubric (frozen excerpt)

Source: gstack `devex-review/SKILL.md`, sections "DX First Principles" through "TTHW Benchmarks",
copied verbatim from `robford-brookai/gstack` at commit `7c9df1c` (v1.60.1.0) on 2026-09-04.
This file is covered by `CHECKSUMS`. Editing it is a rubric change and needs its own PR; it is
never edited inside a `devex-*` change.

Internal-repo interpretation, stated up front so scores are not adjusted after the fact:

- "Community & Ecosystem" for an internal monorepo means a named owner per area (`CODEOWNERS`),
  a channel or person to ask named in `README.md`, issue and PR templates, and evidence that
  someone other than the owner has landed a connector. The last cannot be manufactured.
- "Credible" includes deprecation and upgrade discipline for the connector kit specifically.
- "Findable" includes the docs site, the nav, and whether a grep for the obvious phrase lands
