...
| cpt-om CPT revenue model | both | inbound: command API, via the pulse billing engine (`packages/billing`) — the connector-pattern rule port is cpt-om's registered implementation; outbound: the published warehouse surfaces per `docs/contracts/publishes.md` | the billing engine's per-service credential `billing-engine`; the cpt-om model version travels as `rule_version` on every verdict | one verdict (+ paired transition per `transition_by_outcome`) per rule evaluation, event-driven | spec-only | ownership decided 2026-08-31 (connector-pattern task 1.1, Rob): the dbt verdict models encode cpt-om's qualification logic, so the engine's ported rule modules ARE cpt-om — one owner, no separate direct-declare credential. Declares qualification only — no monetary value crosses in, per `billing-computation-boundary`. Supersedes this row's earlier direct-declare shape (`billing-computation-boundary` design decision 1) |
| Pulse billing engine (`packages/billing`) | declares in | command API, evaluating rules event-driven off its own bus subscription (connector-pattern change) | per-service credential `billing-engine`, actor_type `system`; `rule_version` = `pulse-<verdict_type>-v1` per ported rule module | one verdict (+ paired transition) per rule evaluation | spec-only | the in-pulse implementation of the cpt-om row above — one system, two registry rows only until the connector-pattern change ships, then the cpt-om row's Notes are the pointer; amount-free boundary applies at the package seam |
| Billy | declares in | command API, connector not yet built | planned per-service credential, actor `billy` (D9-pattern external system of record) | TBD — one row per Billy time-entry/episode transition (design pending) | planned | satellite billing app; recorded as an external SoR the same way Customer.io is, once a connector exists. Decided 2026-09-08: Billy also derives the patient × payer digest for the verdict mart at adjudication — it is the mart-row producer of record for that key, and no raw payer identifier reaches the mart (`docs/contracts/consumes.md`) |
...
| `packages/billing-connector` | declares in | command API, consuming its own inbound queue (`BILLING_CONNECTOR_QUEUE_URL`) — the first connector built on the shared kit (`pulse_core.connector`) | per-service credential named by `BILLING_CONNECTOR_TOKEN` (name only; the value is never read outside `declare.py`) | one verdict (+ paired transition per `transition_by_outcome`) per registered `billing.rules.registry` verdict type, per queued fact change | spec-only | reference connector for `docs/connectors/authoring.md`; declares qualification only, no monetary value crosses its seam, per `billing-boundary.md` |
