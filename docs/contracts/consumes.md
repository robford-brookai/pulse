...
|---|---|---|---|
| Verdict mart | dbt-computed Snowflake table(s), produced by `brookai/data-platform` (`management/models/billing/verdict/`) — note `STREAMLINE` in the mart's address is the Snowflake database name, not the producing repo | fixture-pinned contract: one row per `(subject_id, verdict_type, run)`, columns `subject_id, verdict_type, outcome, reason, rule_version, as_of, lineage_ref, computed_at` | a row missing a contract column or carrying an unparseable `as_of`/`computed_at` fails the run before any declaration, naming the offending row; data-platform has no publisher contract of its own yet — this repo's fixtures are the only pinned shape. **Publisher contract drafted 2026-09-08** on `brookai/data-platform` branch `pulse-verdict-mart-publisher-contract` (`management/models/billing/verdict/README.md`), PR pending owner review — pulse could not verify the cited `management/models/billing/verdict/` path or a commit `ffe9770` exist in `brookai/data-platform`'s history as of this entry; the model computing this mart today is `brookai/streamline`'s `dbt_project/models/ocean/marts/ocean_verdicts.sql` (merged `Brookai/streamline#20`, DNA-1252) |
...
Since `billing-state`, a registered verdict type carries a `transition_by_outcome` entry and the
declaration is a pair: the verdict, then a `declare_transition` moving the same subject to the
mapped state. That raises the stakes on two mart columns the relay already read. `outcome` now
decides state, not just evidence, so a mart that mislabels an outcome writes wrong billing or
