...
**Status:** **Accepted** 2026-08-01 (Tal sign-off on Option C topology and the absorption decision, §2) — repo reconciliation complete (§9); all four §9.1 failures closed (V3 → EventBridge migration; V5 → shared `EventBridgePublisher` in `libs/ocean-broker` (DNA-737, superseding the interim convention + CI schema check); V7 → named migration M1, §6.2; V11 → filtered import, §6.1). Delivery is the OpenSpec change `ocean-eventbridge-migration` (DNA-733).
...
| V5 | Producer libraries or emit helpers exist that PULSE-subject producers currently use | libs/ | Producer policy enforced by convention + CI schema check instead of library change | **FAILS.** No shared emit library. `libs/ocean-broker` exports only `build_producer_config` / `build_consumer_config`; each service carries its own duplicated `publish()` (`services/*/src/producer.py`). Producer policy must be enforced by convention plus a CI schema check, per the if-wrong branch. Superseded — the migration built the shared publisher; see §9.1. |
...
**V5 — RESOLVED 2026-08-02: a shared publisher now exists.** V5 initially landed on its if-wrong branch — producer policy by convention plus a CI schema check. The EventBridge migration superseded that branch: task 2.2 (DNA-737) of `ocean-eventbridge-migration` replaced `libs/ocean-broker`'s Kafka config builders with a shared `EventBridgePublisher`, and wave 2b converted every publish site to it. Producer policy is now enforced in a library, which is what V5 originally assumed — the assumption became true by construction rather than by discovery.

**V8 is cheap.** It lands on its stated if-wrong branch with no replanning: convenience replay is deferred to ledger rebuild.
...
3. [x] **§6.1 absorption steps delivered** 2026-08-02 — DNA-695's extension landed as wave 0 of `ocean-eventbridge-migration`: credential rotation (CCC-15), filtered import to `packages/ocean` at `7bc9d2c` (DNA-734), conform-in-place commits (DNA-735, DNA-779). The source archive with the §7 supersession notice is queued as post-merge `destructive_ops` task 9.3 (CCC-18), gated behind the terraform apply and MSK teardown
