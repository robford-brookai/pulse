...
**Shared context for all three:** PULSE monorepo (path per DNA-695 completion note). Read CLAUDE.md first. S1.1 (`pulse-ledger-core`, DNA-784) is built and merged; the names below are pinned, no longer to-confirm. S1.1 provides the ledger schema, the command API client module (`packages/pulse-core/src/pulse_core/client.py` — `pulse_core.client.PulseCoreClient`, `submit_command(...)` classifying responses `committed | replayed | rejected | transient`), command type definitions generated from the catalog (`pulse_core.generated`), and the idempotency-key convention (D16: `{writer_id}:{sha256(subject, command_type, payload, logical_time)}`, derived client-side by `pulse_core.idempotency.derive_idempotency_key`). Command endpoints: `POST /commands`, `POST /commands:batch` (backfill actor only for backfill types). Auth: per-service credentials per D15 — credential names in Context, values from the environment, never in code or fixtures. No live network calls in any test suite.

**Known gap (tasks 4.3/5.3):** the HTTP path is not yet wired end-to-end to the D16 replay semantics — `pulse_ledger.api` rejects an `idempotency_key` body field as unknown and never echoes `replayed` in the response, so `PulseCoreClient` currently classifies every replay as `committed`. The underlying commit path (`pulse_ledger.idempotency.commit_idempotent`) fully supports both. Tracked as DNA-801; see `docs/contracts/publishes.md` and ADR-0003 (Consequences). S1.2's declarer must not be enqueued before DNA-801 lands, since replay classification is its whole job.
...
### Task
1. `packages/verdict-relay/src/verdict_relay/mart_reader.py` — cursor-based reader over the verdict mart contract, ordered (subject, as_of), cursor persisted via S1.1's writer-state facility — `PUT/GET /writers/{writer_id}/cursor` (path template `pulse_core.cursor.CURSOR_PATH_TEMPLATE`, built with `pulse_core.cursor.cursor_path(writer_id)`, values validated JSON-native by `pulse_core.cursor.validate_cursor`; a writer may touch only its own credential's cursor) — so a crashed run resumes without re-reading.
...
### Context
Package: `packages/identity`. Depends on: S1.1 (command API client, `resolve_referral`, `mint_person`, `attach_identifier` command types), S0.2 (ExternalIdentifier system URI conventions). This service takes a `received` Referral's demographics + source identifiers and either matches an existing Person, mints a new TIDE key, or quarantines as ambiguous — the `received → resolved` transition depends on it from day one of P1, and genesis adjudication reuses it. **v1 is deterministic only:** exact match on (system, value) of any ExternalIdentifier wins outright, else a normalized composite (last name + DOB + sex + first-initial, normalization rules in `matching.md` below) with these outcomes — zero candidates mints, one candidate matches, more than one quarantines. No probabilistic scoring in v1: a wrong auto-merge in a HIPAA system is a reportable event, so ambiguity always goes to a human. Evidence model: every resolution command carries the matched-on fields, the rule id, and candidate set size. Quarantine = the Referral stays in `received` with a `resolution_hold` fact (committable with no `to_state` — the write path skips transition validation and the state re-fold) and a row in `ledger.review_queue` (S1.1 schema; `pulse_ledger.review.quarantine_subject` enqueues — a subject is pending at most once — `list_review_queue`/`count_pending` read, and `resolve_review` drains, recording the resolving event id, reviewer role only), drained by the quarantine reviewer role.

**The ledger never receives demographics.** The normalized composite is PHI, and the ledger stores only its **sha256 digest**: `ledger.person_match_keys` carries a `[0-9a-f]{64}` check constraint, and `register_match_key` refuses anything else. Normalization and hashing therefore stay in this package, which remains the only component holding the readable composite — do not write the matcher expecting to send demographics to the ledger, and note that candidate sets in the review queue hold pseudonymous person keys only, so a reviewer follows those keys back to this service's own evidence record.
...
2. `packages/identity/src/identity/matcher.py` — the two-tier deterministic match against the ledger's identity read surface (`pulse_ledger.identity.lookup_identifier(conn, system=..., value=...)` for the exact `(system, value)` tier — uniqueness is the `external_identifiers` primary key — and `pulse_ledger.identity.find_candidates(conn, match_key)` over `ledger.person_match_keys` sha256 digests for the composite tier), returning a typed decision: `Match(person_id, evidence)`, `Mint(evidence)`, `Ambiguous(candidates, evidence)`.
...
4. `packages/identity/src/identity/service.py` — consumption entrypoint: processes `referral.received` events from the backbone via the S1.1 consumer convention — `pulse_core.client.consume(handler, queue_url=...)`, where `handler` is a `pulse_core.client.ConsumerHandler` (`Callable[[Mapping[str, object]], None]` receiving the envelope dict; SQS receive/process/delete with `event_id` dedupe, delete only after the handler returns) — one referral per invocation, safe under redelivery.
